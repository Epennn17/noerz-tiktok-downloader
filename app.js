/* =========================================================
   ...
   API: tiktokv5 (support video + foto slide + music + stats)
   ========================================================= */
...
(function () {
  ...
  function renderResult(data) {
    ...
    if (providerEl) {
      providerEl.textContent = isVideo ? 'via TikTok' : 'via TikTok (Foto Slide)';
    }
    ...
    if (isVideo) {
      ...
      if (thumbWrap) {
        ...
        const videoUrl = downloads[0] || null;
        if (videoUrl) {
          thumbWrap.innerHTML =
      ...
      if (slideWrap) {
        ...
        /* Ambil URL unik (response biasanya punya duplikat mirror CDN) */
        ...
        if (grid) {
          ...
          uniqueImages.forEach(function (url, i) {
            grid.innerHTML +=
              '<a href="' + escapeHtml(url) + '" target="_blank" rel="noopener" class="slide-item" download>' +
                '<img src="' + escapeHtml(url) + '" alt="Slide ' + (i+1) + '" loading="lazy">' +
  ...
  /* Ambil URL gambar unik dari array download
     Response biasanya berisi duplikat mirror CDN (p16, p19, dll) */
  function getUniqueImages(urls) {
    if (!urls || !urls.length) return [];

    /* Coba deteksi hash unik di dalam URL */
    const seen = new Set();
    const result = [];

    urls.forEach(function (url) {
      /* Pola: /photomode-sg/[HASH]~ atau /photomode-sg/[HASH]. */
    ...
    /* Kalau hasilnya > 20, kemungkinan duplicate tidak terdeteksi.
       Fallback: ambil index genap (biasanya pola p16/p19) */
    if (result.length > 20) {
  ...
  function renderDownloadButtons(data) {
    ...
    if (isVideo) {
      /* Video: biasanya downloads[0] = HD, downloads[1] = SD (mirror) */
      if (downloads.length >= 1) {
        hasAny = true;
        grid.innerHTML +=
          '<a href="' + escapeHtml(downloads[0]) + '" target="_blank" rel="noopener" class="btn-download video full" download>' +
            "<i class='bx bx-video'></i> Download Video (HD · No Watermark)" +
          '</a>';
      }
      if (downloads.length >= 2) {
        grid.innerHTML +=
          '<a href="' + escapeHtml(downloads[1]) + '" target="_blank" rel="noopener" class="btn-download video full" download>' +
            "<i class='bx bx-video'></i> Download Video (SD · No Watermark)" +
          '</a>';
      }
    }

    /* Audio: selalu tersedia kalau music.url ada */
    if (music.url) {
      hasAny = true;
      grid.innerHTML +=
        '<a href="' + escapeHtml(music.url) + '" target="_blank" rel="noopener" class="btn-download audio full" download>' +
